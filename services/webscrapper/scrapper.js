...
function getLowestDate(dates) {
  var lowest = dates[0].date;
  for (var i = 1; i < dates.length; i++) {
    if (dates[i].date < lowest) {
      lowest = dates[i].date;
    }
  }
  return lowest;
...
async function scrapeBioBioArticles(browser, targets) {
  ...
  for (const target of targets) {
    const last_date = await getLastArticleDate(target.entity, target.category);
    const daysDiff = moment.duration(moment(current_date).diff(moment(last_date))).asDays();
    if (daysDiff > 1) current_date = new Date().setHours(-24);
...
async function scrapeBioBioBody(browser, articleData) {
  try {
    console.log(`Scrapping ${articleData.article_link}`);

    const page = await browser.newPage();
    await page.setViewport({ width: 800, height: 600 });
    await page.goto(articleData.article_link, { waitUntil: 'load' });

    await page.waitForSelector('article');

    const article = await page.$('article');
    const content = await article.$('div.post-content');

    await page.waitForSelector('div.post-content');

    const ps = await content.$$('p');

    const data = await Promise.all(ps.map(async (p) => {
...
async function saveURLs(data) {
  const { hash_id, title, link, date, image, raw_content, entity, category } = data;
  const fdate = date === '' ? null : date.format();

  try {
    const result = await sql`
      INSERT INTO articles (article_hash, article_title, category, publish_date, raw_content, source_entity, article_link)
      VALUES (${hash_id}, ${title}, ${category}, ${fdate}, ${raw_content}, ${entity}, ${link});
    `;
    return result;
  } catch (e) {
    return null;
...
export const handler = async () => {
  ...
  biobioarticles.map((article) => {
    scrapeBioBioBody(browser, article);
  });
